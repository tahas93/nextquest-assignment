...
const PageMetaData = ({ title }: { title?: string }) => {
  ...
  return (
    <Head>
      ...
      <title>Flirtbate: {title ? title : '#1 Adult Video Call Platform'}</title>
      ...
      <meta name="twitter:image" content={`${WEBSITE_URL}/images/logos/main-logo.svg`} />
      ...
      <meta property="og:image" content={`${WEBSITE_URL}/images/logos/main-logo.svg`} />
      <meta property="og:url" content={`${WEBSITE_URL}${router.asPath}`} />
      <link key="canonical" rel="canonical" href={`${WEBSITE_URL}${router.asPath}`} />
    </Head>
  );
};
